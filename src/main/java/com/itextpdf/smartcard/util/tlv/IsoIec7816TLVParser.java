 ...
 * This is the implementation according to ISO/IEC 7816.
 */
public class IsoIec7816TLVParser implements SimpleTLVParser {
	
	/** Logger instance. */
	private final static Logger LOGGER = LoggerFactory.getLogger(IsoIec7816TLVParser.class);
	
	/**
	 * Parses a byte array and returns a map of tags and values.
	 * Note that we assume that the tag is only one byte long
	 * (which isn't always the case for TLV sequences) and
	 * that the length is calculated as described in ISO/IEC 7816.
	...
	public Map<Byte,String> parse(byte[] data) {
		...
		while (pos < data.length) {
			...
			// If the first byte is not set to 'FF',
			// then the length field consists of a single byte
			length = data[pos++];
			// If the first byte is set to 'FF', then the length
			// field continues on the subsequent two bytes with any
			// value encoding a number from 0 to 65535
			if (length == 0xFF) {
				length = data[pos++] << 8;
				length += data[pos++];
			}
