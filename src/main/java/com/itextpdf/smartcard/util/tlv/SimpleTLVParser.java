...
/**
 * Interface that needs to be implemented by Tag Length Value parsers.
 */
public interface SimpleTLVParser {
	
	/**
	 * Parses a byte array and returns a map of tags and values.
	 * Note that we assume that the tag is only one byte long
	 * (which isn't always the case for TLV sequences).
	 * @param data	the data that needs to be parsed
	 * @return a Map mapping Tags tot content
	 */
